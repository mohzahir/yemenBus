...
<span style="width:100%;color:black;font-size:14px;">  رقم  الجوال بلارقام التالية :1234567890  </span>
    ...
    <form method="post" action="{{route('dashboard.lab.storeSms')}}" class="pb-4 @if ($errors->any()) was-validated @endif" style="margin-bottom:40px;">
        @csrf

        <div class="form-group mt-2">
            <label for="phone">جوال المسافر</label>
            <input type="text" class="form-control" id="phone" name="phone" placeholder="مثال +9661231313131" @if($pcr) value="{{$pcr->phone}}" @endif >
        </div>
        <div class="form-group mt-2">
            <label for="y_phone"> رقم الجوال اليمني</label>
            <input type="text" class="form-control" id="y_phone" name="y_phone" placeholder="مثال +9661231313131" @if($pcr) value="{{$pcr->y_phone}}" @endif >
        </div>
