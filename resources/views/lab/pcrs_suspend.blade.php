...
<div class="main_container col-md-12 col-md-8 col-sm-12 col-xs- " style="margin-left:200px;">
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">  طلبات الفحص قيد المتابعة </li>
  </ol>
</nav>
<h1 style="text-align:center">  طلبات الفحص المستفيدين قيد المتابعة </h1>
...
<table id="myTable" class="table table-striped table-bordered" style="width:100%;text-align:center">
            ...
            @foreach ($pcrs as $pcr)
                ...
                <tr>  
                   ...
                   <td>{{ $pcr->price }}</td>
                       ...
                       <a  data-fancybox="gallery" style=" color: black; text-decoration: none;" class="primary-btn" href="{{asset('public/images/pcrs/pcrs_checked//'.$pcr->done_img)}}">عرض الصورة</a>
                       <br><br>
                       @endif
                        <td> {{ $pcr->note_take }} <br> {{ $pcr->note_check }} </td>

                       </td>

                  
                    <td style="width:150px;margin-top:30px">
                    <a class="btn btn-sm btn-primary" href="{{ route('dashboard.lab.sms',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-envelope"></span>ارسال رسالة </a>
      <a class="btn btn-sm btn-success" @if($pcr->phone) href="https://api.whatsapp.com/send?phone={{$pcr->phone}}" @else href="https://api.whatsapp.com/send?phone={{$pcr->y_phone}}" @endif  style="width:100px">واتس اب </a>
                    ...
                    <a class="btn btn-sm btn-success" href="{{ route('dashboard.lab.checkedForm',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-check"></span>تم الفحص  </a>
                            @endif
                         @if($pcr->status=="1" &&$pcr->done_img)
                    <a class="btn btn-sm btn-primary" href="{{ route('dashboard.lab.share',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-envelope"></span>مشاركه نتيجة الفحص </a>
