...
<table id="myTable" class="table table-striped table-bordered" style="width:100%;text-align:center">
        ...
        <tbody>
            ...
            @foreach ($pcrs as $pcr)
                ...
                <tr>  
                   ...
                   <td>
                       @if($pcr->passport_image)<a  data-fancybox="gallery" style=" color: black; text-decoration: none;" class="primary-btn" href="{{asset('public/images/pcrs/'.$pcr->passport_image)}}">عرض الصورة</a>    @endif
                       <br><br>
                       
                   </td>
                                      <td>{{ $pcr->time_take}}</td>
                                     <td>{{ $pcr->travel_at }}</td>
                                     <td>@if($pcr->day){{ $pcr->dateAdmin() }}@else  لم يتم تحديد الموعد @endif</td>

                    <td style="width:150px;margin-top:30px">
                    <a class="btn btn-sm btn-primary" href="{{ route('dashboard.lab.sms',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-envelope"></span>   راسل المسافر   </a>
      <a class="btn btn-sm btn-success" @if($pcr->phone) href="https://api.whatsapp.com/send?phone={{$pcr->phone}}" @else href="https://api.whatsapp.com/send?phone={{$pcr->y_phone}}" @endif  style="width:100px">واتس اب </a>
                     ...
                     <td style="width:150px;margin-top:30px">
                    <a class="btn btn-sm btn-primary" href="{{ route('dashboard.lab.dateTime',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-calendar"></span>   تحديد موعد الفحص   </a>
                   
                    <a class="btn btn-sm btn-success" href="{{ route('dashboard.lab.takeForm',$pcr->id) }}" style="margin-bottom: 10px"> <span class="glyphicon glyphicon-check"></span>تم أخذ العينة  </a>
