...
@include('flash-message')
    ...
    <form method="post" action="{{route('dashboard.marketers.sendMsms.store')}}" class="pb-4 @if ($errors->any()) was-validated @endif" >
        @csrf
 
<input  name="user_id" value="{{Auth::guard('admin')->user()->id}}" type="hidden">
        
        <div class="form-group mt-2">
            <label for="passenger_phone">كود المسوق</label>
            <input   class="form-control" name="code"   value="{{$marketer->code}}" disabled >
        </div>
        <div class="form-group mt-2">
            <label for="passenger_phone"> رقم جوال السعودي</label>
<input class="form-control" id="phone" name="phone" placeholder="مثال +0598745632" @if($marketer->phone) value="{{$marketer->phone}}" @endif >
        </div>
        <div class="form-group mt-2">
            <label for="y_phone"> رقم الجوال اليمني</label>
            <input type="text" class="form-control" id="y_phone" name="y_phone" placeholder="مثال +759874565"   @if($marketer->y_phone) value="{{$marketer->y_phone}}" @endif >
        </div>
     ...
     <a class="btn btn-danger btn-close btn-lg" href="{{ route('dashboard.marketers.index') }}">اغلاق</a>
