...
<div class="main_container col-md-12 col-md-8 col-sm-12 col-xs- "  style="margin-left:70px;">
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page"> تعديل مختبر</li>
  </ol>
</nav>
   <h1 style ="text-align:center">تعديل مختبر  </h1>
   @include('flash-message')

    <form method="POST" action="{{ route('admin.lab.update',$lab->id) }}"  class="pb-4 @if ($errors->any()) was-validated @endif" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="form-group">
            <label for="name">اسم المختبر</label>
            <input type="text" class="form-control" value="{{$lab->name}}" id="name" name="name"  required>
            @error('name')
        ...
        <div class="form-group">
            ...
            <option value="{{$city->id}}" @if($lab->phone==$city->id) selected @endif>{{$city->name}} </option>
            @endforeach
            </select>
            @error('city_id')
                <div class="invalid-feedback">
        ...
        <div class="form-group" id="discount_percentage-warapper">
            ...
            <input type="text" class="form-control" value="{{$lab->phone}}" id="phone" name="phone"  placeholder="مثال +123456789"  required>
            @error('phone')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
...
<div class="form-group" id="discount_percentage-warapper">
        ...
        <div class="form-group">
            ...
            <option value="{{$p}}" {{ ($lab->priority == $p )? "selected" : "" }}>بدون اولوية </option>
