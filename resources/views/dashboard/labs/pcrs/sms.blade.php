...
@include('flash-message')
    ...
    <form method="post" action="{{route('admin.pcrs.smsSend',$pcr->id)}}" class="pb-4 @if ($errors->any()) was-validated @endif" >
        @csrf
        ...
        <div class="form-group mt-2">
            ...
            <input type="text" class="form-control" id="phone" name="phone" placeholder="مثال 0501234567"   @if($pcr->phone) value="{{$pcr->phone}}" @endif >
        </div>
        <div class="form-group mt-2">
            <label for="y_phone"> رقم الجوال اليمني</label>
            <input type="text" class="form-control" id="y_phone" name="y_phone" placeholder="مثال 701234567"   @if($pcr->y_phone) value="{{$pcr->y_phone}}" @endif >
        </div>
