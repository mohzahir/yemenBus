...
<div x-data="{
    ...
    <form action="{{ route('marketer.reservations.storetransfer') }}" method="post"  id="myForm" class="pb-4 @if ($errors->any()) was-validated @endif" style=" margin-bottom:40px;" >
        @csrf
        ...
        <div class="row">
            <div class="col-md-">
                <div class="form-group">
                    ...
                    <p>  رحلة من {{ $reservation->trip->takeoff_city->name }} الى {{ $reservation->trip->arrival_city->name }} - السعر {{ $reservation->trip->price }}  - التاريخ {{ $reservation->trip->from_date }} - بالرقم {{ $reservation->trip->trip_id }} </p>
                </div>
            </div>
        </div>
        <div class="row">
                ...
                <div class="form-group">
                    ...
                    <select x-on:change="getTripData($el.value)" name="trip_id" id="trip_id" class="form-control" required>
                        ...
                        @foreach($trips as $trip)
                            <option  @if(old('trip_id') == $trip->trip_id) selected @endif value="{{ $trip->trip_id }}">رحلة من {{ $trip->takeoff_city->name }} الى {{ $trip->arrival_city->name }} - السعر {{ $trip->price }}  - التاريخ {{ $trip->from_date }} - بالرقم {{ $trip->trip_id }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
