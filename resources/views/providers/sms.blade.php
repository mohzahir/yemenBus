...
<h2 style="text-align:center">ارسال رسالة</h2>
...
@include('flash-message')
    ...
    <form method="post" action="{{route('provider.storeSms')}}" class="pb-4 @if ($errors->any()) was-validated @endif"  style="margin-bottom:40px;">
        @csrf
 <input type="hidden" class="form-control" id="provider_id" name="user_id"  value="{{Auth::guard('provider')->user()->id}}" >
        ...
        <div class="form-group mt-2">
            ...
            <input type="text" class="form-control" id="passenger_phone_yem" name="passenger_phone_yem" placeholder="مثال +9671231313131"  @if($reservation) value="{{$reservation->passenger_phone_yem}}" @endif>
        </div>
        @if($reservation)
        ...
        <div class="form-group mt-2">
            ...
            <input type="number" min="1" class="form-control" id="amount" name="amount" @if($reservation) value="{{$reservation->amount}}" @endif  required>
        </div>
        @endif
