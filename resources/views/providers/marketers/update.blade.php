...
<div class="main_container col-md-12 col-md-8 col-sm-12 col-xs- " style="margin-left:70px;">
    ...
    <form method="get" action="{{ route('provider.marketer.update',$marketer->id) }}" class="pb-4 @if ($errors->any()) was-validated @endif" enctype="multipart/form-data" style="margin-bottom:40px;">
        @csrf
        <div class="form-group" id="discount_percentage-warapper">
            <label for="name"> اسم المسوق</label>
            <input type="text" class="form-control" id="name" name="name" value="{{$marketer->name}}">
            @error('phone')
